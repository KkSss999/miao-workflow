...
`engine.signal()` 只写一条信号。但「这条信号是给哪一次等待的」必须回答清楚，
否则会出现：**给 gate1 的第二次批准被 gate2 冒领**（没人批准过 gate2，它却过了）。

可用性规则：

```
名字一致
AND ( 定向了 → signal.stepId === 等待的 stepId
   OR 未定向 → signal.seq > stepRun.waitSinceSeq )
```

`waitSinceSeq` 是**等待登记那一刻的信号队列水位线**（`signals.watermark(runId)`），
`seq` 由 storage 单调分配。

为什么不用时间戳：墙钟有同毫秒碰撞，而且**发信号的进程和写等待的进程时钟可能不一致**
（多实例部署下这是常态）。序列号是唯一可信的先后顺序。

不满足条件的信号留在队列里（`countPending` 可见），可以重发或定向投递。
这个方向的取舍是刻意的：**「某个待处理信号没人消费」比「错误批准」可恢复得多。**
